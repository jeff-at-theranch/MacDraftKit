...
enum MD70EllipseDecoder {
    ...
    static func decode(
        ...
        let top = reader.float64BE(at: topOffset).map {
            $0 / storageScale
        }

        let left = reader.float64BE(at: leftOffset).map {
            $0 / storageScale
        }

        let bottom = reader.float64BE(at: bottomOffset).map {
            $0 / storageScale
        }

        let right = reader.float64BE(at: rightOffset).map {
            $0 / storageScale
        }
